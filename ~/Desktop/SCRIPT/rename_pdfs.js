...
// Fonction pour formater le numéro de mois avec un zéro devant si nécessaire
function formatMonth(month) {
  return month.toString().padStart(2, '0');
}

// Fonction pour formater l'année (2 derniers chiffres)
...
// Fonction pour lister tous les fichiers PDF dans un dossier
function getPDFFiles(directory) {
  try {
    const files = fs.readdirSync(directory);
    return files.filter(file => 
      file.toLowerCase().endsWith('.pdf') && 
      !file.startsWith('.') // Exclure les fichiers cachés
    );
  } catch (error) {
...
// Fonction principale pour renommer les fichiers
function renamePDFs(directory) {
  ...
  // Simulation d'une confirmation (en production, vous pourriez utiliser readline)
