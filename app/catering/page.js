...
export default function MenuShabbat() {
  ...
  const menuData = {
    ...
    sides: [
      ...
      { name: "Jerusalem BBQ Mix Grill", description: "Cote de boeuf, meguez, kefta d'agneau et noix d'entrecote. Uniquement à domicile. *min 10 personnes. (85 € par personne)", price: 85 },
  ...
  const addSalade = (salade) => {
    if (selectedFormule) {
      ...
      // Vérifier si le minimum de commande est atteint (en excluant Jerusalem BBQ Mix Grill)
      const totalSides = Object.entries(sideQuantities).reduce((sum, [name, qty]) => {
        const side = menuData.sides.find(s => s.name === name);
        // Exclure Jerusalem BBQ Mix Grill du calcul du minimum
        if (name === "Jerusalem BBQ Mix Grill") {
          return sum;
        }
  ...
  const updateSideQuantity = (sideName, quantity) => {
    setSideQuantities(prev => {
      ...
      // Appliquer le minimum seulement si on passe de 0 à 1
      if (sideName === "Hrayimi le poisson malin" && quantity > 0 && (prev[sideName] || 0) === 0) {
        newQuantity = 3;
      } else if (sideName === "Mloukhia" && quantity > 0 && (prev[sideName] || 0) === 0) {
        newQuantity = 3;
      } else if (sideName === "Jerusalem BBQ Mix Grill" && quantity > 0 && (prev[sideName] || 0) === 0) {
        newQuantity = 10;
      }
      
      // Empêcher de descendre en dessous du minimum
      if (sideName === "Hrayimi le poisson malin" && newQuantity < 3) {
        newQuantity = 3;
      } else if (sideName === "Mloukhia" && newQuantity < 3) {
        newQuantity = 3;
      } else if (sideName === "Jerusalem BBQ Mix Grill" && newQuantity < 10) {
        newQuantity = 10;
      }
      
      const newQuantities = {
        ...prev,
        [sideName]: newQuantity
      };
  ...
  const getSideQuantity = (sideName) => {
    ...
    if (sideName === "Jerusalem BBQ Mix Grill") {
      return sideQuantities[sideName] || 0;
    }
    return sideQuantities[sideName] || 0;
  };
  ...
  const validateFormule = () => {
    ...
    if (!selectedFormule) {
      // Calculer le total en excluant Jerusalem BBQ Mix Grill du minimum
      const totalSidesForMinimum = Object.entries(sideQuantities).reduce((sum, [name, qty]) => {
        const side = menuData.sides.find(s => s.name === name);
        // Exclure Jerusalem BBQ Mix Grill du calcul du minimum
        if (name === "Jerusalem BBQ Mix Grill") {
          return sum;
      ...
      if (totalSidesForMinimum + 15 < 200) {
        alert(`Commande minimum de 200€ avec frais de livraison requis. Votre commande actuelle: ${(totalSidesForMinimum + 15).toFixed(2)}€`);
        return;
      } else {
  ...
  return (
    <div className="min-h-screen bg-[#f9f7f2] text-black menu-shabbat-page">
      ...
      <div className="bg-accent-red text-black py-1 overflow-hidden">
        <div className="animate-marquee whitespace-nowrap">
          <span className="inline-block px-4 font-light text-sm">
            Pour les groupes de plus de 12 personnes merci de nous contacter <a href="https://wa.me/33756872352" className="underline hover:text-gray-700">ici</a>
          </span>
          <span className="inline-block px-4 font-light text-sm">
            Pour les groupes de plus de 12 personnes merci de nous contacter <a href="https://wa.me/33756872352" className="underline hover:text-gray-700">ici</a>
          </span>
      ...
      <div className="bg-[#f9f7f2] py-2 mt-1">
        <div className="container mx-auto px-4 text-center">
          ...
          <h1 className="text-2xl md:text-3xl font-bold -mb-4 text-black">Catering</h1>
        </div>
      </div>
      ...
      <div className="px-[26px] py-3">
        ...
        <div id="formules-section" className="w-full mb-4">
          <h2 className="text-xl font-bold text-black mb-4 text-center">Combos</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {Object.entries(formules).map(([key, formule]) => (
              <div key={key} className="border-b border-gray-800 pb-2">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <h3 className="font-bold text-black">Couscous {formule.personnes} personnes</h3>
                  ...
                  <div className="flex items-center space-x-2 ml-4">
                    ...
                    >
                      {selectedFormule === parseInt(key) ? '✓' : 'Combo+'}
                    </button>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center space-x-2 ml-4">
                    {selectedFormule ? (
                      ...
                      >
                        {selectedCouscous.some(c => c.name === item.name) ? '✓' : 'F'}
                      </button>
                    ) : (
                      <button
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center space-x-2 ml-4">
                    {selectedFormule ? (
                      ...
                      >
                        {selectedSalades.some(s => s.name === item.name) ? '✓' : 'F'}
                      </button>
                    ) : (
                      <button
